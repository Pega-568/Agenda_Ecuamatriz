...
- **Notificaciones Dinámicas (Piloto)**: Se añadió un sistema de notificaciones que combina eventos persistidos en base de datos con alertas virtuales calculadas en tiempo de ejecución (ej. "Empieza en 15 min"). En la aplicación, se ha añadido un Tab "Notificaciones" en el `HomeScreen` para visualizar estas alertas de manera unificada.
- **Alineación Visual (Stitch)**: La paleta de colores y componentes visuales han sido ajustados a los diseños de referencia generados en Stitch (`Desaing_Reference/`), estableciendo coherencia con la versión web.
